...
{
    $res=0;
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT bv as tbv FROM user_id WHERE uid='$uid'"));
    if($r1['tbv'] >= 500)
    {
        $res=1;
    }
...
{
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(pv) as tpv FROM child_counter WHERE uid='$uid'"));
...
function GetBPVAll($db,$pair_amount)
{
    ...
    while ($r1=mysqli_fetch_assoc($q1)) {
        $uid=$r1['uid'];
         $mpv=0;
         $lpv=round($r1['left_bv']/$pair_amount);              
         $rpv=round($r1['right_bv']/$pair_amount);
         if($lpv<$rpv || $lpv==$rpv)
         {
            $mpv=$lpv;
         }
...
function GenerateGTBPayout($db)
{
    ...
    echo "--------------------------------";*/
    extract($res);
    $sql=mysqli_query($db,"SELECT uid,pv FROM child_counter WHERE  pv > 0");
    if(mysqli_num_rows($sql) > 0)
    {
        while ($r1=mysqli_fetch_assoc($sql))
        {
            ...
            if(isset($is_active) && $is_active==1)
            {
                ...
                {
                    $sql1=mysqli_query($db,"SELECT uid,gtb FROM `payout` WHERE uid='$uid' AND cleared=2");
                    if(mysqli_num_rows($sql1) > 0)
                    {
                        $r2=mysqli_fetch_assoc($sql1);
                        $amount=$r2['gtb']+$pamount;
                        $q2=mysqli_query($db,"UPDATE `payout` SET `gtb`= $amount WHERE uid='$uid' and cleared=2");
                    }
                    else
                    {
                        //echo "INSERT INTO `payout` SET uid='$uid',`gtb`= $pamount,`date`='$crdate',cleared=2";
                        $q2=mysqli_query($db,"INSERT INTO `payout` SET uid='$uid',`gtb`= $pamount,`date`='$crdate',cleared=2");
                    }
                }
                if($q2)
                {
                    ...
                    $c[] = 1;
                    $ubv=$pv*500;
                    $q3=mysqli_query($db,"UPDATE `child_counter` SET `pv`= 0,`left_bv`=left_bv-$ubv,`right_bv`=right_bv-$ubv WHERE uid='$uid'");
                    $q4=mysqli_query($db,"INSERT INTO gtb_history SET uid='$uid',pv='$pv',amount='$pamount',gtb_point='$gtb_point',total_turnover='$total_turnover',total_pv='$total_pv',perc='$perc',date='$crdate'");
                }
                else
                {
                    $c[] = 0;
...
function GetPointValueAll($db,$pair_amount,$rank_id)
{
    ...
    $q1=mysqli_query($db,"SELECT t1.uid,t1.rleft_bv,t1.rright_bv FROM `child_counter` t1 LEFT JOIN `pairing` t2 on t1.uid=t2.uid WHERE t2.rank='$rank_id'");
    while ($r1=mysqli_fetch_assoc($q1)) {
        $uid=$r1['uid'];
...
function getRankPayout($db)
{
    ...
    $rr=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `payout_date` where id=1"));
    $pdate=$rr['date'];
    //echo "SELECT sum(bv) as tbv FROM checkout WHERE uid='$uid' and DATE(date) > DATE($pdate)";
    $br=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as tbv FROM checkout WHERE date > '$pdate'"));
    
    $total_turnover_com=$br['tbv'];
    $q1=mysqli_query($db,"SELECT * FROM `child_counter` WHERE rleft_bv > 0 AND rright_bv > 0");
    while ($r1=mysqli_fetch_assoc($q1)) 
    {
        $uid=$r1['uid'];
        $bankres=GetBankDetail($db,$uid);
        ...
        $left_bv=$r1['rleft_bv'];
        $right_bv=$r1['rright_bv'];
        if($r1['rleft_bv']>=$r1['rright_bv'])
        {
            $mcount=$r1['rright_bv'];
        }
        else
        ...
        //echo "SELECT * FROM `rank_payout_detail` WHERE `bv`<=$mcount";
        $q22=mysqli_query($db,"SELECT * FROM `rank_payout_detail` WHERE `bv`<=$mcount");
        //var_dump($q2);
        $rno=mysqli_num_rows($q22);
        if($rno > 0)
        {
            while ($r2=mysqli_fetch_assoc($q22)) 
            {
                //echo $amount="<br><br>bv-->".$r2['bv']." percentage ---> ".$r2['perc'];
                $colname=$r2['colname'];
                $tbl=$colname."_history";
                $rank_id=$r2['rank_id'];
                $tbv=$r2['bv'];
                $bonus_perc=$r2['perc'];
                $total_point=GetPointValueAll($db,$tbv,$rank_id);
                $res1=GetRankPoint($db,$tbv,$bonus_perc,$total_point,$total_turnover_com);
                //var_dump($res1);echo "__________<br><br>";
                if(isset($res1['bonus_point_value']) > 0)
                {
                    $mypoint1=$mcount/$tbv;
                    $mypoint=floor($mypoint1);
                    extract($res1);
                    
                    //echo "<br>  amount --- ".$pamount=$bonus_point_value*$mypoint;
                    if(isset($bankres) && $bankres==1)
                    {
                        $sql1=mysqli_query($db,"SELECT uid,$colname FROM `payout` WHERE uid='$uid' AND cleared=0");
                        if(mysqli_num_rows($sql1) > 0)
                        {
                            $r2=mysqli_fetch_assoc($sql1);
                            $pamount=$r2[$colname]+$pamount;
                            
                            $q2=mysqli_query($db,"UPDATE `payout` SET `$colname`= $pamount WHERE uid='$uid' and cleared=0");
                        }
                        else
                        {
                            $q2=mysqli_query($db,"INSERT INTO `payout` SET uid='$uid',`$colname`= $pamount,`date`='$crdate'");
                        }
                    }
                    else
                    {
                        $sql1=mysqli_query($db,"SELECT uid,$colname FROM `payout` WHERE uid='$uid' AND cleared=2");
                        if(mysqli_num_rows($sql1) > 0)
                        {
                            $r2=mysqli_fetch_assoc($sql1);
                            $pamount=$r2[$colname]+$pamount;
                            
                            $q2=mysqli_query($db,"UPDATE `payout` SET `$colname`= $pamount WHERE uid='$uid' and cleared=2");
                        }
                        else
                        {
                            $q2=mysqli_query($db,"INSERT INTO `payout` SET uid='$uid',`$colname`= $pamount,`date`='$crdate',cleared=2");
                        }
                    ...
                    if($q2)
                    {
                        ...
                        {
                            $uname=GetUname($db,$uid);
                            $msg = "Payout Generated: ".date('d/m/Y H:i:s')."
                             User : $uname
                             Amount : $pamount";
                            if(isset($msg) && $msg!='' && !empty($msg))
                            {
                        ...
                        $c[] = 1;
                        $q4=mysqli_query($db,"INSERT INTO `$tbl` SET uid='$uid',pv='$mypoint',amount='$pamount',bonus_point='$bonus_point_value',total_turnover='$total_turnover',total_pv='$total_point',perc='$perc',left_bv='$left_bv',right_bv='$right_bv',`date`='$crdate'")or die(mysqli_error($db));
    ...
    $q3=mysqli_query($db,"UPDATE `child_counter` SET `rleft_bv`=0,`rright_bv`=0 WHERE 1") or die(mysqli_error($db));
    return $c;
}
