    ...
    $id=$txtid; 
        ...
        if($check_pan>0){
         $msg = "Pan no error! This pan no is already available plz try diffrent.";  
            ...
            /*$rr1=mysqli_fetch_assoc(mysqli_query($db,"SELECT t1.position FROM `pairing` t1 LEFT JOIN user_id t2 on t1.uid=t2.uid WHERE t2.uname='$sponsor_id'"));
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {  
                                    ...
                                    if($cpin==1)
                                    {   
                                        $qry="INSERT INTO `user_id`(`uname`, `password`, `register_date`, `pin`,`paired`) VALUES ('$username','$password','$cdate','$pin_no','$paired')";
                                        $q=mysqli_query($db,$qry) or die(mysqli_error($db));
                                        if($q)
                                        {
                                            $uid=mysqli_insert_id($db);
