    ...
    $puid=isset($data['puid']) && !empty($data['puid'])?$data['puid']:$user_id;
        ...
        if($q1)
        {
           ...
           if(isset($position) && $position=='L')
            {
                mysqli_query($db,"UPDATE child_counter SET left_pv=left_pv+$pv,rleft_pv=rleft_pv+$pv WHERE uid='$sponsor_id'");
                //mysqli_query($db,"UPDATE child_counter SET left_bv=left_bv+$pv,rleft_bv=rleft_bv+$pv WHERE uid='$parent_id'");
            }
            if(isset($position) && $position=='R')
            {
                mysqli_query($db,"UPDATE child_counter SET right_pv=right_pv+$pv,rright_pv=rright_pv+$pv WHERE uid='$sponsor_id'");
                //mysqli_query($db,"UPDATE child_counter SET right_bv=right_bv+$pv,rright_bv=rright_bv+$pv WHERE uid='$parent_id'");
            }
            PVCount($db,$sponsor_id,$pv);
            PVCount_parent($db,$puid,$pv);
            DecreementPVCount($db,$puid,$pv);
            Commision_distribute($db,$puid,$pv,$hbt=0,$puid);
            ...
            $order_id= "QFRM".leftpad($transaction_id,6);
