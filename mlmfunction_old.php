...
{
    $res=0;
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as tbv FROM checkout WHERE uid='$uid'"));
    if($r1['tbv'] >= 500)
    {
        $res=1;
    }
...
{
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as tbv FROM checkout WHERE uid='$uid'"));
    return isset($r1['tbv']) && $r1['tbv'] > 0?$r1['tbv']:0;
}
function GetUserTotalPV($db,$uid)
{
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as tpv FROM checkout WHERE uid='$uid'"));
    $tpv=$r1['tpv']*50;
...
function GetUserBV($db,$uid)
{
    ...
    $pdate=$rr['date'];
    //echo "SELECT sum(bv) as tbv FROM checkout WHERE uid='$uid' and date > '$pdate'";
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as tbv FROM checkout WHERE uid='$uid' and date > '$pdate'"));
    return $r1['tbv'];
}
function GetUserRank($db,$uid)
{
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT t1.rank,t2.bv FROM `pairing` t1 left join self_purchase t2 on t1.rank=t2.rank_id WHERE t1.uid='$uid'"));
    return array("rank"=>$r1['rank'],"bv"=>$r1['bv']);
...
//     if($res)
//     {
//         $tbv=GetUserBV($db,$uid);
//         $res2=GetUserRank($db,$uid);
//         $rbv=$res2['bv'];
//         /*var_dump($res);
//         echo "tbv---".$tbv."-----rbv----".$rbv;*/
//         if(isset($tbv) && isset($rbv) && $tbv >= $rbv)
//         {
//             $result=1;
//         }
//     }
...
function PVCount($db,$uid,$bv = null){
    ...
    if(mysqli_num_rows($query)>0){
        ...
        if($row['position']=="L")
        {
            mysqli_query($db,"UPDATE child_counter SET left_bv=left_bv+$bv,rleft_bv=rleft_bv+$bv WHERE uid='$parent_id'");
        }
        else if($row['position']=="R")
        {
            mysqli_query($db,"UPDATE child_counter SET right_bv=right_bv+$bv,rright_bv=rright_bv+$bv WHERE uid='$parent_id'");
        }
        if($parent_id==1)
        {
             ...
             //$childid=$row['parent_id'];
             BVCount($db,$parent_id,$bv);
         }
    }
    return true;
}

function BVCount_parent($db,$uid,$bv = null){
  ...
  $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM pairing  WHERE uid='$uid'"));
    ...
    if(mysqli_num_rows($query)>0){
        ...
        if($parent_id==1)
        {
             ...
             //$childid=$row['parent_id'];
             BVCount_parent($db,$parent_id,$bv);
         }
    }
  return true;
}

function DecreementBVCount_parent($db,$uid,$bv = null){
  ...
  $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM pairing  WHERE uid='$uid'"));
    ...
    if(mysqli_num_rows($query)>0){
        ...
        if($parent_id==1)
        {
             ...
             //$childid=$row['parent_id'];
             DecreementBVCount_parent($db,$parent_id,$bv);
         }
    }
  return true;
}

function DecreementBVCount($db,$uid,$bv = null){
    ...
    if(mysqli_num_rows($query)>0){
        ...
        if($parent_id==1)
        {
             ...
             //$childid=$row['parent_id'];
             DecreementBVCount($db,$parent_id,$bv);
         }
    }
    return true;
}
...
function getTotalBV($uid_array = array()){
    ...
    if(!empty($uid_array)){
        if(!empty($uid_array[0])){
            ...
            $pdate=$rr['date'];
            $uids = "'".implode("','",$uid_array)."'";
            $sql1 = "SELECT IFNULL(sum(bv),0) as tbv FROM checkout WHERE date > '$pdate' and uid IN($uids)";
            $query1 = mysqli_query($db,$sql1) or die(mysqli_error($db));
            $row1 = mysqli_fetch_assoc($query1);
            $total_business = $row1['tbv'];
        }
    }
    return $total_business;
}
...
function selfpurchase_BV($db,$uid,$left_bv,$right_bv){
 $rr=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `payout_date` where id=1"));    
 $pdate=$rr['date'];
 $self_purchase_bv=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as self_bv FROM checkout WHERE uid='$uid' && date > '$pdate'"));
         if($self_purchase_bv['self_bv']>0){
             if($left_bv<$right_bv){
              $lpv_self= $self_purchase_bv['self_bv'];
            }
            elseif($right_bv<$left_bv){
...
//     $q1=mysqli_query($db,"SELECT uid,left_bv,right_bv,rleft_bv,rright_bv FROM `child_counter` where uid!=1");
//     while ($r1=mysqli_fetch_assoc($q1)) {
//         $uid=$r1['uid'];
//         $is_active=GetMonthlyPurchaseStatus($db,$uid);
//         $self_purchase_bv=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as self_bv FROM checkout WHERE uid='$uid' && user='$uid'"));
        
//         if(isset($is_active) && $is_active==1){
//             if($self_purchase_bv['self_bv']>0){
//              if($r1['left_bv']<$r1['right_bv']){
//               $lpv_self= $self_purchase_bv['self_bv'];
//             }
//             elseif($r1['right_bv']<$r1['left_bv']){
//               $rpv_self= $self_purchase_bv['self_bv'];  
//             }  
//             else{
//               $lpv_self= $self_purchase_bv['self_bv']; 
...
//         $llpp=$r1['left_bv']+$lpv_self;
//         $rrpp=$r1['right_bv']+$rpv_self;
//          $mpv=0;
...
//          }
//          $rmpv=0;
//          $rlpv=round($r1['rleft_bv']/$pair_amount);              
//          $rrpv=round($r1['rright_bv']/$pair_amount);
...
function GetBPVAll($db,$pair_amount)
{
    ...
    while ($r1=mysqli_fetch_assoc($q1)) {
        ...
        $self=selfpurchase_BV($db,$uid,$r1['left_bv'],$r1['right_bv']);
        extract($self);
        $llpp=$r1['left_bv']+$lpv_self;
        $rrpp=$r1['right_bv']+$rpv_self;
        ...
        // echo"<br>";
         ...
         $tot_pv+=$mpv;             
           ...
           if($fbv['left_bv'] > $fbv['right_bv']){
              $right_bv=0;
              $left_bv = $fbv['left_bv'];
           }
           else{
              $left_bv=0;
              $right_bv = $fbv['right_bv'];
           }
           if($fbv['rleft_bv'] > $fbv['rright_bv']){
              $rright_bv=0;
              $rleft_bv = $fbv['rleft_bv'];
           }
           else{
              $rleft_bv=0;
              $rright_bv = $fbv['rright_bv'];
           }
           mysqli_query($db,"UPDATE `child_counter` SET `left_bv`=$llefttBv,`right_bv`=$rrightBv WHERE uid='$uid'");
        }
...
function GetGTBPoint($db)
{
    ...
    //$res=UpdateRank($db);
    //$br=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as tbv FROM `user_id` WHERE uid!='1'"));
    $rr=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `payout_date` where id=1"));
    //$pdate=date("Y-m-d", strtotime($rr['date']));
    $pdate=$rr['date'];
    //echo "SELECT sum(bv) as tbv FROM `checkout` WHERE uid!='1' and DATETIME(date) > DATETIME('$pdate')";
    $br=mysqli_fetch_assoc(mysqli_query($db,"SELECT sum(bv) as tbv FROM `checkout` WHERE uid!='1' and date > '$pdate'"));
    $total_turnover_com=$br['tbv'];
    $bonus_perc=35;
...
{
    global $crdate;
    $sql=mysqli_query($db,"SELECT uid,left_bv,right_bv,pv FROM child_counter WHERE  left_bv > 0 and right_bv> 0 and uid!=1");
    if(mysqli_num_rows($sql) > 0)
    {
        while ($r1=mysqli_fetch_assoc($sql))
        {   
          ...
          if($user_active > 0 ){
            ...
            if(mysqli_num_rows($previous_bv_transaction)>0){
              mysqli_query($db,"UPDATE `previous_bv_transaction` SET `left_bv`=left_bv+'$left_bv',`right_bv`=right_bv+'$right_bv' WHERE `uid`='$uid'");
            }
            else{
                mysqli_query($db,"INSERT `previous_bv_transaction` SET `uid`='$uid', `left_bv`='$left_bv',`right_bv`='$right_bv'");
            }
            // end
            ...
            if($q2)
            {
                ...
                $lefttBv=$addselfbv['left_bv'];
                $rightBv=$addselfbv['right_bv'];
                $l_bv=$addselfbv['left_bv']-$pay_bv;
                $r_bv=$addselfbv['right_bv']-$pay_bv;
                ...
                $q3=mysqli_query($db,"UPDATE `child_counter` SET `left_bv`=$l_bv1,`right_bv`=$r_bv1 WHERE uid='$uid'");
...
function UpdateRank($db) 
{
    ...
    while ($r1=mysqli_fetch_assoc($q1)) 
    {
       ...
       $q2=mysqli_query($db,"SELECT * FROM `child_counter` WHERE rleft_bv>=$tpv && rright_bv>=$tpv");
       if(mysqli_num_rows($q2) > 0)
       {
           while ($r2=mysqli_fetch_assoc($q2)) 
           {
...
function GetPointValueAll($db,$pair_amount,$rank_id)
{
    ...
    $q1=mysqli_query($db,"SELECT t1.uid,t1.rleft_bv,t1.rright_bv FROM `child_counter` t1 LEFT JOIN `userAchieveRank` t2 on t1.uid=t2.uid WHERE t2.rank_id='$rank_id'");
    while ($r1=mysqli_fetch_assoc($q1)) {
        $uid=$r1['uid'];
        $is_active=GetMonthlyPurchaseStatus($db,$uid);  
        $self=selfpurchase_BV($db,$uid,$r1['rleft_bv'],$r1['rright_bv']);
        if(isset($is_active) && $is_active==1){
            extract($self);
            $llpp=$r1['rleft_bv']+$lpv_self;
            $rrpp=$r1['rright_bv']+$rpv_self;
...
function GetUserPointValue($db,$pair_amount,$uid)
{
    ...
    $q1=mysqli_query($db,"SELECT t1.uid,t1.rleft_bv,t1.rright_bv FROM `child_counter` t1 LEFT JOIN `pairing` t2 on t1.uid=t2.uid WHERE t2.uid='$uid'");
    while ($r1=mysqli_fetch_assoc($q1)) {
        $uid=$r1['uid'];
         $mpv=0;
         $lpv=round($r1['rleft_bv']/$pair_amount);              
         $rpv=round($r1['rright_bv']/$pair_amount);
         if($lpv<$rpv || $lpv==$rpv)
         {
            $mpv=$lpv;
         }
...
function GetRankPoint($db,$tbv,$bonus_perc,$total_point,$total_turnover_com)
{
    $bonus_point_value=0;
    $bonus=($total_turnover_com*$bonus_perc)/100;
    //echo $bonus;
...
function getRankPayout($db)
{
    global $crdate;
     ...
     // Faiz previous bv saved
       ...
       while ($get_pv=mysqli_fetch_array($get_pv12)) {
            $uids=$get_pv['uid'];
            $left_pv=$get_pv['rleft_bv']*50;
            $right_pv=$get_pv['rright_bv']*50;
    ...
    mysqli_query($db,"UPDATE `user_id` SET `bv`=0");
    mysqli_query($db,"UPDATE `child_counter` SET `rleft_bv`=0,`rright_bv`=0 WHERE `rleft_bv`>0 and `rright_bv`>0") or die(mysqli_error($db));
...
function checkAppreciationFortNightly($db){
   ...
   while ($row=mysqli_fetch_array($users)) { 
     ...
     $rleft_bv=$row['rleft_bv'];
     $rright_bv=$row['rright_bv'];
     $getFN_date=$pairing['fn_date'];
     $achieve_rank=$pairing['achieve_rank'];
     $cr_date=date('Y-m-d');
     if($pairing['rleft_bv'] > 0 && $pairing['rright_bv']>0){
        $pair_rleft_bv=$row['rleft_bv']-$pairing['rleft_bv'];
        $pair_rright_bv=$row['rleft_bv']-$pairing['rright_bv'];
     }
     else{
        $pair_rleft_bv=0;
        $pair_rright_bv=0;
     ...
     if($achieve_rank==1){
        if(($rleft_bv >= 20 && $rright_bv >= 20) && ($rleft_bv <=250 && $rright_bv<=250) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 20 && $pair_rright_bv >= 20) && ($pair_rleft_bv <= 250 && $pair_rright_bv <= 250) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/20);
            $tot_rright_bv=floor($rright_bv/20);
     ...
     elseif($achieve_rank==2){
        if(($rleft_bv >= 250 && $rright_bv >= 250) && ($rleft_bv <=500 && $rright_bv<=500) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 250 && $pair_rright_bv >= 250) && ($pair_rleft_bv <= 500 && $pair_rright_bv <= 500) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/250);
            $tot_rright_bv=floor($rright_bv/250);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==3){
        if(($rleft_bv >= 500 && $rright_bv >= 500) && ($rleft_bv <=1000 && $rright_bv<=1000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 500 && $pair_rright_bv >= 500) && ($pair_rleft_bv <= 1000 && $pair_rright_bv <= 1000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/500);
            $tot_rright_bv=floor($rright_bv/500);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==4){
        if(($rleft_bv >= 1000 && $rright_bv >= 1000) && ($rleft_bv <=2500 && $rright_bv<=2500) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 1000 && $pair_rright_bv >= 1000) && ($pair_rleft_bv <= 2500 && $pair_rright_bv <= 2500) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/1000);
            $tot_rright_bv=floor($rright_bv/1000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==5){
        if(($rleft_bv >= 2500 && $rright_bv >= 2500) && ($rleft_bv <=5000 && $rright_bv<=5000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 2500 && $pair_rright_bv >= 2500) && ($pair_rleft_bv <= 5000 && $pair_rright_bv <= 5000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/2500);
            $tot_rright_bv=floor($rright_bv/2500);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==6){
        if(($rleft_bv >= 5000 && $rright_bv >= 5000) && ($rleft_bv <=10000 && $rright_bv<=10000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 5000 && $pair_rright_bv >= 5000) && ($pair_rleft_bv <= 10000 && $pair_rright_bv <= 10000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/5000);
            $tot_rright_bv=floor($rright_bv/5000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==7){
        if(($rleft_bv >= 10000 && $rright_bv >= 10000) && ($rleft_bv <=20000 && $rright_bv<=20000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 10000 && $pair_rright_bv >= 10000) && ($pair_rleft_bv <= 20000 && $pair_rright_bv <= 20000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/10000);
            $tot_rright_bv=floor($rright_bv/10000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==8){
        if(($rleft_bv >= 20000 && $rright_bv >= 20000) && ($rleft_bv <=40000 && $rright_bv<=40000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 20000 && $pair_rright_bv >= 20000) && ($pair_rleft_bv <= 40000 && $pair_rright_bv <= 40000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/20000);
            $tot_rright_bv=floor($rright_bv/20000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==9){
        if(($rleft_bv >= 40000 && $rright_bv >= 40000) && ($rleft_bv <=80000 && $rright_bv<=80000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 40000 && $pair_rright_bv >= 40000) && ($pair_rleft_bv <= 80000 && $pair_rright_bv <= 80000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/40000);
            $tot_rright_bv=floor($rright_bv/40000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==10){
        if(($rleft_bv >= 80000 && $rright_bv >= 80000) && ($rleft_bv <=160000 && $rright_bv<=160000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 80000 && $pair_rright_bv >= 80000) && ($pair_rleft_bv <= 160000 && $pair_rright_bv <= 160000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/80000);
            $tot_rright_bv=floor($rright_bv/80000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==11){
        if(($rleft_bv >= 160000 && $rright_bv >= 160000) && ($rleft_bv <=320000 && $rright_bv<=320000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 160000 && $pair_rright_bv >= 160000) && ($pair_rleft_bv <= 320000 && $pair_rright_bv <= 320000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/160000);
            $tot_rright_bv=floor($rright_bv/160000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==12){
        if(($rleft_bv >= 320000 && $rright_bv >= 320000) && ($rleft_bv <=640000 && $rright_bv<=640000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 320000 && $pair_rright_bv >= 320000) && ($pair_rleft_bv <= 640000 && $pair_rright_bv <= 640000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/320000);
            $tot_rright_bv=floor($rright_bv/320000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==13){
        if(($rleft_bv >= 640000 && $rright_bv >= 640000) && ($rleft_bv <=1280000 && $rright_bv<=1280000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 640000 && $pair_rright_bv >= 640000) && ($pair_rleft_bv <= 1280000 && $pair_rright_bv <= 1280000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/640000);
            $tot_rright_bv=floor($rright_bv/640000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==14){
        if(($rleft_bv >= 1280000 && $rright_bv >= 1280000) && ($rleft_bv <=2560000 && $rright_bv<=2560000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 1280000 && $pair_rright_bv >= 1280000) && ($pair_rleft_bv <= 2560000 && $pair_rright_bv <= 2560000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/1280000);
            $tot_rright_bv=floor($rright_bv/1280000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
     ...
     elseif($achieve_rank==15){
        if(($rleft_bv >= 2560000 && $rright_bv >= 2560000) && ($getFN_date==NULL )){
         $FN_date1   = $fort_N_date;
         mysqli_query($db,"UPDATE `pairing` SET `appriciation_FN` = 1 , `fn_date`= '$FN_date1' , `rleft_bv`='$rleft_bv' , `rright_bv`='$rright_bv' WHERE uid=$uid");
         }         
         elseif(($pair_rleft_bv >= 2560000 && $pair_rright_bv >= 2560000) && $getFN_date==$cr_date){
            $tot_rleft_bv=floor($rleft_bv/2560000);
            $tot_rright_bv=floor($rright_bv/2560000);
            if($tot_rleft_bv == $tot_rright_bv){
                $total_noOF_time=$tot_rleft_bv;
            }
            elseif($tot_rleft_bv < $tot_rright_bv){
...
function get_currentRankCommision($db,$rank,$uid,$bv){
 $user_active=IsDistributor($db,$uid);
 if($user_active > 0 ){   
...
function Commision_distribute($db,$uid,$bv = null,$hbt,$self_id=null){
    $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT `sponsor_id` FROM pairing  WHERE uid='$uid'"));
    ...
    if(mysqli_num_rows($query)>0){
      ...
      //}
        if($sponsor_id==1)
        {
           ...
           Commision_distribute($db,$sponsor_id,$bv,$hbt);
         }
...
function Is_userActive($db,$uid,$bv){
