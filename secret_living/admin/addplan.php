									...
									<div class="row">
										<div class="form-group">
											...
											<div class="col-md-2">
												...
												<div class="fancy-form">
													<div class="fancy-form fancy-form-select">
													 <select class="form-control select2" name="plan_name" required="">
													 <?php
													 $getPackage=mysqli_query($db,"SELECT * FROM `packages`");
													 while($package = mysqli_fetch_array($getPackage)){
													 ?>
													   <option value="<?= $package['id'] ?>" <?php echo isset($data['pack_id']) && $data['pack_id']==$package['id']?'selected':''; ?>><?= $package['title'] ?></option>
													 <?php } ?>
													 </select>
                                            ...
                                            <div class="col-md-2">
                                                <label>PV*</label>
                                                <div class="fancy-form"><!-- input -->
                                                    <i class="fa" style="font-size: 13px;">PV</i>
                                                    <input type="number"  placeholder = "Enter PV" value="<?php echo isset($data['pv'])?$data['pv']:''?>" class = "form-control" name = "pv" id = "pv" title = "Enter PV!"  required>
                                                </div>
                                            </div>
