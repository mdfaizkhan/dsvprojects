                ...
                <div class="panel-body ">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover sample_5" id="sample_5">
                            ...
                            where (
                                (SELECT IFNULL(SUM(`direct_sponsor_comm`),0) FROM `payout` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`level_comm`),0) FROM `payout` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`repurchase_double_pairing_bonus`),0) FROM `payout` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`double_pairing_bonus`),0) FROM `payout` WHERE `cleared` = 0 and  `uid` = t1.uid) >0
                            )";
