		...
		<li id="main_id"  data-id='<?php echo $q1["paired"];?>'>
			...
			$parent_id=$q1["uid"];
				...
				echo "<ul>";
					...
					$lq1=mysqli_query($db,"SELECT t1.*,t2.* from user_id t1 LEFT join user_detail t2 on t1.uid = t2.uid left join pairing t3 on t1.uid=t3.uid where t3.parent_id = '$parent_id' and t1.uid!=1");
				$label_1st = mysqli_num_rows($lq1);
				if($label_1st > 0)
				{
                	while ($l1 = mysqli_fetch_assoc($lq1))
                	{
						...
						else
						{
							...
							";
							$lq=mysqli_query($db,"SELECT t1.*,t2.* from user_id t1 LEFT join user_detail t2 on t1.uid = t2.uid left join pairing t3 on t1.uid=t3.uid where t3.parent_id = '$npid'");
							$label_2nd = mysqli_num_rows($lq);
							if($label_2nd > 0)
							{	
								echo "<ul>";
								...
								if(!empty($npid))
								{
		                			...
		                			while ($l = mysqli_fetch_assoc($lq)) {
										...
										else
										{
											...
											$total_child_count1 = total_child_count( $db, $l["paired"] );
											//list( $nofleft, $nofright) = explode("_", $total_child_count);
