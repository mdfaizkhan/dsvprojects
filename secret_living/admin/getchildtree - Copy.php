...
<ul style='min-width:2000px;' >
    ...
    <li id="main_id"  data-id='<?php echo $q1["paired"];?>'>
        ...
        $pair = $q1["paired"]+1;
        $parent_id=$q1["uid"];
        //echo "SELECT t1.*,t2.* from user_id t1 join user_detail t2 on t1.uid = t2.uid left join pairing t3 on t1.uid=t3.uid where t3.parent_id = '$parent_id' and t1.uid!=1";
            ...
            if(!empty($parent_id))
            {
                ...
                while ($l1 = mysqli_fetch_assoc($lq1))
                {
                    ...
                    else
                    {
                        ...
                        $total_child_count = total_child_count( $db, $l1["paired"] );
                        //list( $nofleft, $nofright) = explode("_", $total_child_count);
                    ...
                    echo "<ul>";
                    $lq=mysqli_query($db,"SELECT t1.*,t2.* from user_id t1 join user_detail t2 on t1.uid = t2.uid left join pairing t3 on t1.uid=t3.uid where t3.parent_id = $npid and t1.uid!=1");
        ...
        else
        {
            echo "<li class='tree-user' >
                        <div class='alert-danger'>
                        <a href='register.php?parentid=".$q1["uname"]."'>
                          <span class='add-icon'><i class='fa fa-plus'></i></span>
                          <h4>Add</h4>
                         </a>
                         </div>
