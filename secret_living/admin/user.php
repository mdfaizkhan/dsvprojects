			...
			<div class="panel-body ">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover sample_5" id="sample_5">
						...
						$sql=mysqli_query($db,"SELECT t1.uid as ttid,t1.*,t2.*,t3.parent_id,t3.sponsor_id,t4.first_name as pfname,t4.last_name as plname,t5.uname as puname,ub.bank_name,ub.acnumber from user_id t1 LEFT join user_detail t2 on t1.uid =t2.uid LEFT join user_bank ub on t1.uid=ub.uid join pairing t3 on t1.uid=t3.uid left join user_detail t4 on t3.sponsor_id=t4.uid left join user_id t5 on t3.sponsor_id=t5.uid where t1.role = 'user' order by t1.uid");
							...
							<tr class="odd gradeX">
								...
								<td  class="">
								  <?php
								   if($row['is_active']==1){ 
								  ?>
								  <a class="btn btn-success btn-xs" onclick="unblock_user(<?php echo $uid1;?>)">Un Block</a>
								  <?php
								   }
								   else{
								  ?>
								  <a class="btn btn-danger btn-xs" onclick="block_user(<?php echo $uid1;?>)">Block</a>
								  <?php } ?>
									
									<a href="addusers?id=<?php echo $uid1; ?>" class="btn btn-xs btn-info" title="Edit User"><i class="fa fa-pencil"></i> Edit</a>
									<a href="welcomeletter?id=<?php echo $row['uid']; ?>" target="_blank" class="btn btn-xs btn-success" title="Welcome Letter"><i class="fa fa-eye"></i> View</a>
									<a href="changepassuser?id=<?php echo $uid1;; ?>" class="btn btn-xs btn-warning" title="Change Password"><i class="fa fa-key"></i> Change Password</a>
