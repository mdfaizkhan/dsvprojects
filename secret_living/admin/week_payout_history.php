<?php include("header.php");?>
    ...
    <section id="middle">
        <header id="page-header">
            <h1>Generated Previous Payout</h1>
        </header>
            ...
            <div id="panel-1" class="panel panel-default">
                <div class="panel-heading">
                    <span class="title elipsis">
                        <strong>Generated Previous Payout</strong> <!-- panel title -->
                ...
                <div class="panel-body ">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover sample_5" id="sample_5">
                            ...
                            t2.pan_no,
                            t3.*,
                            (SELECT IFNULL(SUM(`gtb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as a,
                            (SELECT IFNULL(SUM(`dsb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as b,

                            (SELECT IFNULL(SUM(`eb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as c,
                            (SELECT IFNULL(SUM(`db`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as d,
                            (SELECT IFNULL(SUM(`sdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as e,
                            (SELECT IFNULL(SUM(`gdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as f,
                            (SELECT IFNULL(SUM(`ddb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as g,
                            (SELECT IFNULL(SUM(`reward`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as h,
                            
                            (SELECT IFNULL(SUM(`pdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as i,
                            (SELECT IFNULL(SUM(`cdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) as j

                            FROM user_id t1 
                            LEFT JOIN user_bank t3 ON t1.uid =t3.uid 
                            LEFT JOIN user_detail t2 ON t1.uid =t2.uid AND t1.uid!=1 
                            where t1.uid!=1 and (
                                (SELECT IFNULL(SUM(`gtb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`dsb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`eb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`db`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`sdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`gdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 OR
                                (SELECT IFNULL(SUM(`ddb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0  OR
                                (SELECT IFNULL(SUM(`pdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0  OR
                                (SELECT IFNULL(SUM(`cdb`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0  OR
                                (SELECT IFNULL(SUM(`reward`),0) FROM `payout_history` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 
                            )";
                            ...
                            while($row = mysqli_fetch_assoc($sql))
                            {
                                ...
                                $s=$row['a']+$row['b']+$row['c']+$row['d']+$row['e']+$row['f']+$row['g']+$row['h']+$row['i']+$row['j'];
                                $e = ($s*5/100);
                                $f = (($s-$e)*5/100);
