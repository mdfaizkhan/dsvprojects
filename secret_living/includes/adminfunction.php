...
if (isset($_POST["type"]) && $_POST["type"] == "ManagePlan")
{
    ...
    $pack=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `packages` WHERE `id`='$plan_name'"));
    $planame=$pack['title'];
    ...
    if (isset($plan_id) && !empty($plan_id))
    {
        ...
        $q1 = mysqli_query($db, "UPDATE `plans` SET  `title`='$title',`plan_name`='$planame',`plan_amount`='$plan_amount',`plan_desc`='$plan_desc',`levels`='$levels',`pack_id`='$plan_name',`pv`='$pv' WHERE `plan_id` = '$plan_id'");
        if ($q1)
        {
            $success = true;
    ...
    {
        //$binary_com_percentage = ($binary_com*$plan_amount)/100;
        $q1 = mysqli_query($db, "INSERT INTO `plans` SET `title`='$title',`plan_name`='$planame',`plan_amount`='$plan_amount',`plan_desc`='$plan_desc',`levels`='$levels',`pack_id`='$plan_name',`pv`='$pv'") or die(myslqi_error($db));
        if ($q1)
        {
            $success = true;
...
if (isset($_POST['type']) && $_POST['type'] == "Managesetting")
{
    $success = false;
    $message = "";
    $stm_update = ''; //Image
    ...
    $q1 = mysqli_query($db, "UPDATE `setting` SET $stm_update WHERE `id` = '$id'");
    if ($q1)
    {
        $success = true;
