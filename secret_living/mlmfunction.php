...
function setting($db){
 $setting=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `setting` WHERE `id`=1"));
 return $setting;
}
...
function FirstRegisterPayment($db,$uid,$parent_id,$is_sponsor)
{
    ...
    if($amount > 0)
    {
        ...
        if(mysqli_num_rows($q2)>0)
        {
             ...
             $q1 = mysqli_fetch_assoc(mysqli_query($db,"SELECT t3.pv FROM transpin t2  join plans t3 on t2.plan_id = t3.plan_id and t2.uid = '$uid'"));
             PVCount_parent($db,$uid,$q1['pv']);  
            ...
            $amount1=$amount*40/100;
            $bqry3=mysqli_query($db,"select id,uid,direct_sponsor_comm from `payout` WHERE uid='$parent_id' and cleared=0") or die(mysqli_error($db));
            $qq_no=mysqli_num_rows($bqry3);
            if($qq_no > 0)
            {      
                $brow=mysqli_fetch_assoc($bqry3);
                $pid=$brow['id'];                
                $q2=mysqli_query($db,"UPDATE `payout` SET `direct_sponsor_comm`= direct_sponsor_comm+$amount1 WHERE uid='$parent_id' and id='$pid'");
            }
            else
            {
                $q2=mysqli_query($db,"INSERT INTO `payout` SET `uid`= $parent_id,`direct_sponsor_comm`= $amount1,`date`='$crdate'");
            }
        }
...
{
    global $crdate;
    $LevelPercentNo=count($levelPercent);
...
function AddFinalAmount($db,$uid,$amount,$is_sponsor)
{
    ...
    $bqry3=mysqli_query($db,"select id,uid,level_comm from `payout` WHERE uid='$uid' and cleared=0") or die(mysqli_error($db));
    $qq_no=mysqli_num_rows($bqry3);
    if($qq_no > 0)
    {      
        $brow=mysqli_fetch_assoc($bqry3);
        $pid=$brow['id'];                
        $q2=mysqli_query($db,"UPDATE `payout` SET `level_comm`= level_comm+$amount WHERE uid='$uid' and id='$pid'");
    }
    else
    {
        $q2=mysqli_query($db,"INSERT INTO `payout` SET `uid`= $uid,`level_comm`= $amount,`date`='$crdate'");
    }
    return;
}
...
function PVCount_parent($db,$uid,$pv = null){
  ...
  $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM pairing  WHERE uid='$uid'"));
    ...
    if(mysqli_num_rows($query)>0){
        ...
        {
            // mysqli_query($db,"UPDATE child_counter SET left_pv=left_pv+$pv,rleft_pv=rleft_pv+$pv WHERE uid='$parent_id'");
            mysqli_query($db,"UPDATE child_counter SET left_pv=left_pv+$pv WHERE uid='$parent_id'");
        }
        else if($row['position']=="R")
        {
            mysqli_query($db,"UPDATE child_counter SET right_pv=right_pv+$pv WHERE uid='$parent_id'");
        }
        if($parent_id==1)
        {
    ...
    if(mysqli_num_rows($sql) > 0)
    {
        ...
        {   
           $uid=$r1['uid'];
            ...
            $pamount=$pay_bv*$setting['pool1_double_pairing_bonus']/100;                      
            $sql1=mysqli_query($db,"SELECT uid,double_pairing_bonus FROM `payout` WHERE uid='$uid' AND cleared=0");
            if(mysqli_num_rows($sql1) > 0)
            {
                $r2=mysqli_fetch_assoc($sql1);
                $amount=$r2['double_pairing_bonus']+$pamount;
                $q2=mysqli_query($db,"UPDATE `payout` SET `double_pairing_bonus`= $amount WHERE uid='$uid' and cleared=0");
            }
            else
            {
                $q2=mysqli_query($db,"INSERT INTO `payout` SET uid='$uid',`double_pairing_bonus`= $pamount,`date`='$crdate'");
            }
            ...
            if($q2)
            {
                ...
                $q3=mysqli_query($db,"UPDATE `child_counter` SET `left_pv`=$l_bv,`right_pv`=$r_bv WHERE uid='$uid'");
...
function unilevel_commission($db,$uid,$amount,$sponsor_id){
    global $crdate;
    $is_sponsor=0;
    $levelPercent=getPlanPerc($db,$uid);
    ...
    $lper=$levelPercent[0];
    $pay_amount=($amount * $lper) / 100;
       $q2 = $db->query("SELECT t1.* FROM `user_id` t1  WHERE t1.`uid` = '$sponsor_id' and t1.status=1 ");
    if(mysqli_num_rows($q2)>0)
    {
       $sql=mysqli_query($db,"INSERT INTO `child_earning`(`uid`, `parent_id`, `perc`, `amount`, `date`) VALUES ('$uid','$sponsor_id','$lper','$pay_amount','$crdate')");
       AddFinalAmount($db,$sponsor_id,$pay_amount,$is_sponsor);
    }

    $i=1;
    AllParentAmount($db,$uid,$sponsor_id,$amount,$i,$levelPercent);     
}


function PVCount_parent_repurchase($db,$uid,$pv = null){
  ...
  $r1=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM pairing  WHERE uid='$uid'"));
    ...
    if(mysqli_num_rows($query)>0){
        ...
        if($row['position']=="L")
        {
            mysqli_query($db,"UPDATE child_counter SET rleft_pv=rleft_pv+$pv WHERE uid='$parent_id'");
        }
        else if($row['position']=="R")
        {
            mysqli_query($db,"UPDATE child_counter SET rright_pv=rright_pv+$pv WHERE uid='$parent_id'");
        }
        if($parent_id==1)
        {

        }else{
             PVCount_parent_repurchase($db,$parent_id,$pv);
         }
    }
  return true;
}

function GenerateGTBPayout_repurchase($db)
{
    global $crdate;
    $sql=mysqli_query($db,"SELECT uid,rleft_pv,rright_pv,pv FROM child_counter WHERE  rleft_pv > 0 and rright_pv> 0 and uid!=1");
    if(mysqli_num_rows($sql) > 0)
    {
        while ($r1=mysqli_fetch_assoc($sql))
        {   
           $uid=$r1['uid'];
            ...
            $pamount=$pay_bv*$setting['repurchased_pairing_bonus']/100;                        
            $sql1=mysqli_query($db,"SELECT uid,repurchase_double_pairing_bonus FROM `payout` WHERE uid='$uid' AND cleared=0");
            if(mysqli_num_rows($sql1) > 0)
            {
                $r2=mysqli_fetch_assoc($sql1);
                $amount=$r2['repurchase_double_pairing_bonus']+$pamount;
                $q2=mysqli_query($db,"UPDATE `payout` SET `repurchase_double_pairing_bonus`= $amount WHERE uid='$uid' and cleared=0");
            }
            else
            {
                $q2=mysqli_query($db,"INSERT INTO `payout` SET uid='$uid',`repurchase_double_pairing_bonus`= $pamount,`date`='$crdate'");
            }
            ...
            if($q2)
            {
                ...
                $getRank=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `pairing` WHERE uid='$uid'"));
                $ranks=$getRank['achieve_rank'];
                $lefttBv=$addselfbv['rleft_pv'];
                $rightBv=$addselfbv['rright_pv'];
                $l_bv=$addselfbv['rleft_pv']-$pay_bv;
                $r_bv=$addselfbv['rright_pv']-$pay_bv;              
                

                $q3=mysqli_query($db,"UPDATE `child_counter` SET `rleft_pv`=$l_bv,`rright_pv`=$r_bv WHERE uid='$uid'");
