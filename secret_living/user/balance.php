...
$data=mysqli_fetch_assoc(mysqli_query($db,"SELECT balance FROM `user_id` where uid='$mlmid'"));
?>
<section id="middle">
    <header id="page-header">
        <h1>Wallet Balance Request</h1>
    </header>
    <div id="content" class="padding-20">
        <div id="panel-1" class="panel panel-default mypanel">
            <div class="panel-heading">
                <span class="title elipsis">
                    <strong>Wallet Balance ₹<?=$data['balance']; ?></strong>
            ...
            <div class="panel-body ">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover sample_5" id="sample_5">
                        ...
                        <?php
                        $i = 1;
                        $sql=mysqli_query($db,"SELECT t1.*,t2.uname,t3.first_name,t3.last_name FROM bal_req t1 left join user_id t2 on t1.uid=t2.uid left join user_detail t3 on t1.uid=t3.uid where t1.uid='$mlmid' order by t1.req_id ") or die(mysqli_error($db));
                        ...
                        ?>
                            <tr class="odd gradeX">
                                ...
                                <td>
                                    <?php if($row['status'] == 0)
                                    { ?>
                                  <a href="javascript:void(0);" title="Delete Request" class="DeleteWBReq btn btn-danger btn-xs" data-id="<?php echo $row['req_id'];?>"><i class="fa fa-trash"></i> Delete</a>
                              <?php } ?>
                                </td>
                            </tr>
                        <?php
