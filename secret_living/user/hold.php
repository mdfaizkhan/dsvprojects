                        ...
                        <table class="table table-striped table-bordered table-hover sample_5" id="sample_5">
                            ...
                            //$myquery = "SELECT t1.uname,t1.uid as ttid,t2.first_name,t2.last_name,(SELECT IFNULL(SUM(`amount`),0) FROM `payout` WHERE `cleared` = 0 and  `uid` = t1.uid) as a from user_id t1 join user_detail t2 on t1.uid =t2.uid and ((SELECT IFNULL(SUM(`amount`),0) FROM `payout` WHERE `cleared` = 0 and  `uid` = t1.uid) >0 )";
                             $myquery = "SELECT 
                                        ...
                                        t2.pan_no,
                                        t3.*,
                                        (SELECT IFNULL(SUM(`gtb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as a,
                                        (SELECT IFNULL(SUM(`dsb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as b,

                                        (SELECT IFNULL(SUM(`eb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as c,
                                        (SELECT IFNULL(SUM(`db`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as d,
                                        (SELECT IFNULL(SUM(`sdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as e,
                                        (SELECT IFNULL(SUM(`gdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as f,
                                        (SELECT IFNULL(SUM(`ddb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as g,
                                        (SELECT IFNULL(SUM(`reward`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as h,
                                        
                                        (SELECT IFNULL(SUM(`pdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as i,
                                        (SELECT IFNULL(SUM(`cdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) as j
                                        ...
                                        t1.uid='$mlmid' AND 
                                        (
                                            (SELECT IFNULL(SUM(`gtb`),0) FROM `payout` WHERE `cleared` =2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`dsb`),0) FROM `payout` WHERE `cleared` =2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`eb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`db`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`sdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`gdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`ddb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`pdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`cdb`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 OR
                                            (SELECT IFNULL(SUM(`reward`),0) FROM `payout` WHERE `cleared` = 2 and  `uid` = t1.uid) >0 
                                        ) 
                                        order by t1.uid ";
                            //echo $myquery;
