...
<section id="middle">
	<header id="page-header">
		<h1>Not Completed KYC Member</h1>  
	</header>
	...
	<div id="content" class="padding-20">
		<div id="panel-1" class="panel panel-default mypanel">
			<div class="panel-heading">
				<span class="title elipsis">
					<strong>Not Completed KYC Member List</strong> <!-- panel title -->
				</span>
						...
						if(!empty($users[0])){
					        ...
					        if(mysqli_num_rows($query1) > 0)
					        {
						        while($row = mysqli_fetch_assoc($query1))
			                    {
			                        ...
			                        $uid1 = $row["uid"];
            						$check_purchased=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `user_bank` WHERE `uid`='$uid1'"));
            						$user_bank=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `user_bank` WHERE `uid`='$uid1'"));
            						if($check_purchased['pan_card_image']=='' || $check_purchased['adhar_card_image_front']=='' || $user_bank['acnumber']=='' || $user_bank['bank_name']=='' || $user_bank['branch_name']=='' || $user_bank['branch_name']=='' || $user_bank['swiftcode']==''){
